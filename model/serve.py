# model/serve.py
import json
import uvicorn
import os
from fastapi import FastAPI, HTTPException
from pydantic import BaseModel
from sentence_transformers import SentenceTransformer, util
from transformers import AutoTokenizer, AutoModelForSequenceClassification
import spacy
import torch
import numpy as np
from typing import Dict

# Assuming simplify_sentences.py and silhouet_config.py are in the same directory
import simplify_sentences
from silhouet_config import *

app = FastAPI()

# --- Global Variables & Model Loading ---

# Use CUDA if available
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"

# 1. Sentence Transformer for Personality Scoring
SCORING_MODEL = SentenceTransformer("all-mpnet-base-v2")
SCORING_MODEL.to(DEVICE)

# 2. Models for Claim Extraction
# Load from /models directory as specified in Dockerfile
def load_claim_model_and_tokenizer(local_path):
    tokenizer = AutoTokenizer.from_pretrained(local_path, use_safetensors = True)
    model = AutoModelForSequenceClassification.from_pretrained(local_path)
    return tokenizer, model

DEBERTA_TOKENIZER, DEBERTA_MODEL = load_claim_model_and_tokenizer("/models/claimbuster-deberta-v2")

# 3. SpaCy for Sentence Splitting
spacy.prefer_gpu()
NLP = spacy.load("en_core_web_trf")
SENTENCE_SPLITTER = simplify_sentences.AssertionOptimizedSentenceSplitter(NLP)

# Pre-compute personality vectors
key_vectors = {}
mask_vectors = {}

# Config for scoring
RELEVANCE_SOFT_START = 0.2
RELEVANCE_HARD_CUTOFF = 0.05
SENSITIVITY_FACTORS: Dict[str, float] = {
    "relationship_satisfaction": 0.7,
    "resentment": 0.9,
    # default 1.0 for others
}

for key in PERSONALITY_KEYS:
    pos_examples = [PERSONALITY_LABEL_MAP[key][0]]
    neg_examples = [PERSONALITY_LABEL_MAP[key][1]]

    pos_embeddings = SCORING_MODEL.encode(pos_examples, convert_to_tensor=True, device=DEVICE)
    neg_embeddings = SCORING_MODEL.encode(neg_examples, convert_to_tensor=True, device=DEVICE)

    pos_vector = torch.mean(pos_embeddings, dim=0, keepdim=True)
    neg_vector = torch.mean(neg_embeddings, dim=0, keepdim=True)

    key_vectors[key] = {
        "positive": pos_vector,
        "negative": neg_vector,
    }

    mask_phrases = SENSITIVITY_MASKS.get(key)
    if mask_phrases:
        mask_embeds = SCORING_MODEL.encode(mask_phrases, convert_to_tensor=True, device=DEVICE)
        mask_vectors[key] = mask_embeds

# --- API setup ---

class ProcessRequest(BaseModel):
    text: str

def calculate_scores(text: str, doc_embedding):
    """Calculates personality scores for the given text."""
    final_scores = {key: 0.0 for key in PERSONALITY_KEYS}

    for key in PERSONALITY_KEYS:
        if key in mask_vectors:
            sims = util.cos_sim(doc_embedding, mask_vectors[key]).cpu().numpy().flatten()
            relevance = float(max(sims))
        else:
            relevance = 1.0

        if relevance < RELEVANCE_HARD_CUTOFF:
            continue

        weight = 1.0
        if relevance < RELEVANCE_SOFT_START:
            weight = (relevance - RELEVANCE_HARD_CUTOFF) / (RELEVANCE_SOFT_START - RELEVANCE_HARD_CUTOFF)

        pos_vector = key_vectors[key]["positive"]
        neg_vector = key_vectors[key]["negative"]
        pos_similarity = util.cos_sim(doc_embedding, pos_vector).item()
        neg_similarity = util.cos_sim(doc_embedding, neg_vector).item()
        raw_score = pos_similarity - neg_similarity

        sensitivity = SENSITIVITY_FACTORS.get(key, 1.0)
        adjusted_score = raw_score * weight * sensitivity
        final_scores[key] = adjusted_score

    score_values = np.array(list(final_scores.values()))
    if np.any(score_values != 0):
        exp_scores = np.exp(score_values - np.max(score_values))
        softmax_scores = SCALE_FACTOR * exp_scores / exp_scores.sum()
        return dict(zip(final_scores.keys(), softmax_scores.tolist()))
    else:
        return final_scores

def extract_claims(text: str):
    """Extracts claims from the given text."""
    # Use the sentence splitter to get assertion candidates
    candidates = SENTENCE_SPLITTER.get_assertion_candidates(text)
    sentences = [x['text'] for x in candidates]
    claims = []
    for sent in sentences:
        inputs = DEBERTA_TOKENIZER(sent, return_tensors="pt", truncation=True)
        with torch.no_grad():
            outputs = DEBERTA_MODEL(**inputs)
            scores = torch.softmax(outputs.logits, dim=1).tolist()[0]
        # Assuming the first score is the 'claim' probability
        if scores[0] >= 0.5:
            claims.append({"sentence": sent, "claimbuster_scores": scores})
    return claims

@app.post("/process")
async def process_text(request: ProcessRequest):
    """
    A unified endpoint that scores text for personality and extracts claims.
    """
    text = request.text
    if not text or not text.strip():
        raise HTTPException(status_code=400, detail="No text provided")

    print(f"[Processing] Full text: {text[:80]}...")

    # 1. Generate a single embedding for the entire document for scoring.
    doc_embedding = SCORING_MODEL.encode(text, convert_to_tensor=True, device=DEVICE)

    # 2. Calculate personality scores.
    scores = calculate_scores(text, doc_embedding)

    # 3. Extract claims.
    claims = extract_claims(text)

    # 4. Combine results.
    # Using json.dumps to ensure the response is a JSON string as in the original /score endpoint
    return json.dumps({
        "scores": scores,
        "claims": claims
    })

if __name__ == '__main__':
    uvicorn.run(app, host='0.0.0.0', port=8001)
