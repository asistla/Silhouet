...
async def register_user(
    ...
    user_data = request.model_dump() # Get data from Pydantic request

    # Explicitly set all non-nullable fields that have Python-side defaults
    # or are generated by the backend, ensuring they are in user_data.
    # user_id is set below
    ...
    try:
        ...
        db.rollback()
        # Check for specific unique constraint violations if you add more unique fields
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"Registration conflict: {e.orig}")
    except Exception as e:
        db.rollback()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"User registration failed: {str(e)}")
